...
> v2.0.0 (current): CLI/LaunchAgent flow with configurable balance, ready to be embedded or shipped by third-party apps or installers. Prebuilt Apple Silicon archive: `Release/a11y-balance-mac-2.0.0-macos-arm64.zip`.
...
- Prebuilt v2.0.0 included: `Release/a11y-balance-mac-2.0.0-macos-arm64.zip` (Apple Silicon). Legacy prebuilt v1.0.0 remains available. Double-clicking a binary runs it only for the current session; use the plist/launchctl steps below for autostart at login.
...
- `v2.0.0` - CLI `--balance` support and templated LaunchAgent; third-party apps/installers can embed the daemon and preconfigure balance without touching the source. Apple Silicon prebuilt: `Release/a11y-balance-mac-2.0.0-macos-arm64.zip`. Install via `./Scripts/install.sh` (optionally `BALANCE=0.35 ./Scripts/install.sh`) to set the persisted balance.

## Install v2.0.0 from source (current)
...
## Install v2.0.0 from prebuilt (Apple Silicon)
1) Unzip (from repo root):  
   ```bash
   unzip Release/a11y-balance-mac-2.0.0-macos-arm64.zip -d Release
   ```
2) Install binary and create the plist (choose your balance), from repo root:  
   ```bash
   DEST_BIN="$HOME/.local/bin/a11y-balance"
   BALANCE=0.25  # 0.0=left, 0.5=center, 1.0=right
   mkdir -p "$HOME/.local/bin" "$HOME/Library/LaunchAgents"
   cp Release/a11y-balance-mac-2.0.0/a11y-balance "$DEST_BIN"
   chmod +x "$DEST_BIN"
   sed "s#__BINARY_PATH__#$DEST_BIN#g; s#__BALANCE_VALUE__#$BALANCE#g" \
     Release/a11y-balance-mac-2.0.0/com.a11y.balance.daemon.plist.example \
   ...
   ```
If you are on Intel, rebuild from source on an Intel Mac. Double-clicking the binary only runs it for the current session; to start at login, load the plist via `launchctl` as above.
...
Example with folder/zip name `a11y-balance-mac-2.0.0`:
1) Build in release: `swift build -c release`  
2) Prepare an output folder: `mkdir -p Release/a11y-balance-mac-2.0.0`  
3) Copy the binary and (optional) plist example:  
   ```bash
   cp .build/release/A11yBalance Release/a11y-balance-mac-2.0.0/a11y-balance
   cp LaunchAgent/com.a11y.balance.daemon.plist.example Release/a11y-balance-mac-2.0.0/
   ```  
4) Create the zip: `cd Release && zip -r a11y-balance-mac-2.0.0-macos-arm64.zip a11y-balance-mac-2.0.0`  
